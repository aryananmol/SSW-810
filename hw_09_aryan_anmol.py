...
class University:
    ...
    def _read_students(self, path='students.txt')-> None:
        """read files"""
        try:
            for cwid, name, major in file_reader(os.path.join(path), fields=3,header = True, sep=';'):
    ...
    def _read_instructors(self, path='instructors.txt')-> None:
        """read files"""
        try:
            for insid, name, major in file_reader(os.path.join(path), fields=3,sep='|',header = True):
    ...
    def _read_grades(self,path='grades.txt')-> None:
        """read fles"""
        try:
            for cwid, course_name, grade, insid in file_reader(os.path.join(path), fields=4,sep='|',header = True):
